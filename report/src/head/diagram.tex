...
\def\bctext#1{\Large\textcolor{bccolor}{\ensuremath{\bm{#1}}}}
...
\newcommand{\drawcomp}{%
  ...
  \iftoggle{comptext}{%
    % Nodes
    \iftoggle{complabel}{%
      \node[above, align=center] at ($(rvalve)+(0,\valveheight)$) {Recycle Valve\\\statetext{q_\text{r\complabelsuffix}}};
      \iftoggle{compfirstvalve}{
      ...
      \node[above, align=center] at ($(rvalve)+(0,\valveheight)$) {\statetext{q_\text{r\complabelsuffix}}};
    }


    \iftoggle{compfirstvalve}{
      \draw[<-,draw=inputcolor,line width=2pt] ($(ivalve)-(2*\valveheight,0)$) -- ++(-1,0) node[below]{\inputtext{u_\text{s\complabelsuffix}}};
    }{}

    \draw[<-,draw=inputcolor,line width=2pt] ($(ovalve)-(2*\valveheight,0)$) -- ++(-1,0) coordinate (x);
    \node[below] at (x) {\inputtext{u_\text{d\complabelsuffix}}};
    \draw[<-,draw=inputcolor,line width=2pt] ($(rvalve)-(0,2*\valveheight)$) -- ++(0,-1) coordinate (x);
    \node[right] at ($(x)+(0,0.5)$) {\inputtext{u_\text{r\complabelsuffix}}};
    \draw[<-,draw=inputcolor,line width=2pt] (comp-tl |- comp-br) -- ++(-1,0) coordinate (x);
    \node[above] at (x) {\inputtext{T_\text{d\complabelsuffix}}};

    \iftoggle{complabel}{%
      \node[align=center] at ($0.5*(itank-tr)+0.5*(itank-bl)$) {Suction\\Tank\\\statetext{p_\text{s\complabelsuffix}}};
      \node[align=center] at ($0.5*(otank-br)+0.5*(otank-tl)$) {Discharge\\Tank\\\statetext{p_\text{d\complabelsuffix}}};
      \node[align=center] at ($0.5*(comp-tl)+0.5*(comp-br |- comp-bl)$) {Compressor\\\statetext{\omega_\text{c\complabelsuffix}, q_\text{c\complabelsuffix}}};
    }{%
      \node[align=center] at ($0.5*(itank-tr)+0.5*(itank-bl)$) {\statetext{p_\text{s\complabelsuffix}}};
      \node[align=center] at ($0.5*(otank-br)+0.5*(otank-tl)$) {\statetext{p_\text{d\complabelsuffix}}};
      \node[align=center] at ($0.5*(comp-tl)+0.5*(comp-br |- comp-bl)$) {\statetext{\omega_\text{c\complabelsuffix}, q_\text{c\complabelsuffix}}};
    }

    % Legend
    \iftoggle{compleg}{%
      % boundary conditions
      \node at ($(ivalve) + (0,2)$) {\bctext{p\ut{a}}};
      \node at ($(ovalve) + (0,2)$) {\bctext{p\ut{a}}};
