...
#define NWSR            200						/**< Maximum number of working set recalculations. */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	real_T *out_objVal, *out_xOpt, *out_status, *out_nWSR, *out_cputime;
	...
	if ( count[0] == 0 )
	{
		...
		status = problem->init( H,g,A,lb,ub,lbA,ubA, nWSR,&cputime );
		ssGetPWork(S)[0] = ( void* ) problem;
	...
	else
	{
		...
		status = problem->hotstart( H,g,A,lb,ub,lbA,ubA, nWSR,&cputime );
    ...
    out_cputime = ssGetOutputPortRealSignal(S, 4);
