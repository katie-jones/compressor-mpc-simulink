...
x = x_init_lin;
[A,B,C] = get_linearized_matrices(x, u_init);
f_init = get_comp_deriv(x,u_init)';
...
delta_u = [rand(N,1)*0.002-0.001, zeros(N,2), rand(N,1)*0.05-0.025];
...
%%
[A,B,C,Ac,Bc,Cc] = get_linearized_matrices(x, u_init);

sys = ss(A,B,C,0,0.05);
sysc = ss(A,B,C,0);

figure; step(sys,sysc)
