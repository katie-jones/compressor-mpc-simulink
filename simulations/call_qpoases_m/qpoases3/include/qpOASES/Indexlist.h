...
/**
 *	\file include/qpOASES/Indexlist.h
...
returnValue Indexlist_getNumberArray(	Indexlist* THIS,
 ...
 *	\return SUCCESSFUL_RETURN \n
			RET_INDEXLIST_CORRUPTED */
returnValue	Indexlist_getISortArray(	Indexlist* THIS,
 ...
 *	\return >= 0: Index of given number. \n
 			-1: Number not found. */
int Indexlist_getIndex(	Indexlist* THIS,
...
static inline int Indexlist_getNumber(	Indexlist* THIS,
...
static inline int Indexlist_getLength(	Indexlist* THIS
										);

/** Returns last number within the index list.
 *	\return Last number within the index list. */
static inline int Indexlist_getLastNumber(	Indexlist* THIS
											);
...
returnValue Indexlist_addNumber(	Indexlist* THIS,
									int addnumber			/**< Number to be added. */
									);

/** Removes number from index list.
 *	\return SUCCESSFUL_RETURN */
returnValue Indexlist_removeNumber(	Indexlist* THIS,
									int removenumber	/**< Number to be removed. */
									);

/** Swaps two numbers within index list.
 *	\return SUCCESSFUL_RETURN */
returnValue Indexlist_swapNumbers(	Indexlist* THIS,
...
static inline BooleanType Indexlist_isMember(	Indexlist* THIS,
 ...
 *  bisection.
 *  \return j. */
int Indexlist_findInsert(	Indexlist* THIS,
							int i
							);
 ...
 *	g e t N u m b e r
 */
static inline int Indexlist_getNumber( Indexlist* THIS, int physicalindex )
{
	/* consistency check */
	if ( ( physicalindex < 0 ) || ( physicalindex > THIS->length ) )
		return -RET_INDEXLIST_OUTOFBOUNDS;

	return THIS->number[physicalindex];
}
 ...
 *	g e t L a s t N u m b e r
 */
static inline BooleanType Indexlist_isMember( Indexlist* THIS, int _number )
{
	if ( Indexlist_getIndex( THIS,_number ) >= 0 )
		return BT_TRUE;
	else
		return BT_FALSE;
}
