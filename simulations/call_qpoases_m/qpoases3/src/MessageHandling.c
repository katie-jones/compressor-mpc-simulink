...
#include <qpOASES/MessageHandling.h>
#include <qpOASES/Utils.h>
...
{ RET_ENSURELI_FAILED, "Failed to ensure linear independence of active contraint matrix", VS_VISIBLE },
										...
										)
{
	...
	if ( Enumber <= SUCCESSFUL_RETURN )
		return MessageHandling_throwError( THIS,RET_ERROR_UNDEFINED,0,__FUNCTION__,__FILE__,__LINE__,VS_VISIBLE );

	/* Call to common throwMessage function if error shall be displayed. */
	if ( THIS->errorVisibility == VS_VISIBLE )
		return MessageHandling_throwMessage( THIS,Enumber,additionaltext,functionname,filename,linenumber,localVisibilityStatus,"ERROR" );
	else
		return Enumber;
}
											...
											)
{
	/* consistency check */
  	if ( Wnumber <= SUCCESSFUL_RETURN )
		return MessageHandling_throwError( THIS,RET_WARNING_UNDEFINED,0,__FUNCTION__,__FILE__,__LINE__,VS_VISIBLE );

	/* Call to common throwMessage function if warning shall be displayed. */
	if ( THIS->warningVisibility == VS_VISIBLE )
		return MessageHandling_throwMessage( THIS,Wnumber,additionaltext,functionname,filename,linenumber,localVisibilityStatus,"WARNING" );
  	else
  		return Wnumber;
}
										...
										)
{
	...
	if ( Inumber < SUCCESSFUL_RETURN )
		return MessageHandling_throwError( THIS,RET_INFO_UNDEFINED,0,__FUNCTION__,__FILE__,__LINE__,VS_VISIBLE );

	/* Call to common throwMessage function if info shall be displayed. */
	if ( THIS->infoVisibility == VS_VISIBLE )
		return MessageHandling_throwMessage( THIS,Inumber,additionaltext,functionname,filename,linenumber,localVisibilityStatus,"INFO" );
	else
		return Inumber;
}
...
{
	MessageHandling_setErrorVisibilityStatus(   THIS,VS_VISIBLE );
	MessageHandling_setWarningVisibilityStatus( THIS,VS_VISIBLE );
	MessageHandling_setInfoVisibilityStatus(    THIS,VS_VISIBLE );

	MessageHandling_setOutputFile( THIS,stdFile );
	MessageHandling_setErrorCount( THIS,0 );
											...
											)
{
	...
	int numberOfWhitespaces = (THIS->errorCount-1)*2;
	...
	if ( numberOfWhitespaces >= QPOASES_MAX_STRING_LENGTH )
		numberOfWhitespaces = QPOASES_MAX_STRING_LENGTH-1;
	...
	{
		MessageHandling_throwError( THIS,RET_EWI_UNDEFINED,0,__FUNCTION__,__FILE__,__LINE__,VS_VISIBLE );
		return RETnumber;
	}
